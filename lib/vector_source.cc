...
template <class T>
vector_source<T>::vector_source(const std::vector<T> &data, bool repeat,
                                unsigned int vlen,
                                const std::vector<tag_t> &tags)
    : sync_block("vector_source", io_signature::make(0, 0, 0),
                 io_signature::make(1, 1, sizeof(T) * vlen)),
      d_data(data), d_repeat(repeat), d_offset(0), d_vlen(vlen), d_tags(tags) {
  if (tags.empty()) {
    d_settags = 0;
  } else {
    d_settags = 1;
