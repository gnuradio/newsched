...
template <>
pmt_map2<std::string>::pmt_map2(const std::map<std::string, pmt_wrap>& val) : 
    pmt_base(Data::MapString), 
    _map(val) 
{
    // Don't need anything here.
...
template <>
pmt_map2<std::string>::pmt_map2(const uint8_t* buf, size_t size):
    pmt_base(Data::MapString)
{
    set_buffer(buf, size);
...
template <>
flatbuffers::Offset<void> pmt_map2<std::string>::rebuild_data(flatbuffers::FlatBufferBuilder& fbb)
{
    throw std::runtime_error("This should not get called");
}

template <>
pmt_wrap& pmt_map2<std::string>::operator[](const pmt_map2::key_type& key)
{
    return _map[key];
}

template <>
void pmt_map2<std::string>::fill_flatbuffer()
{
    _fbb.Reset();
...
template <>
void pmt_map2<std::string>::serialize_setup()
{
    fill_flatbuffer();
}
}
