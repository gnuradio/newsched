...
public:
    ...
    void partition(partition_conf_vec& confs)
    {
        ...
        for (auto c : domain_crossings) {
            ...
            auto da_src = domain_adapter_zmq_req_cli::make(std::string("tcp://127.0.0.1:1234"), buffer_location_t::REMOTE, c.src().port());
            auto da_dst = domain_adapter_zmq_rep_svr::make(std::string("tcp://127.0.0.1:1234"), buffer_location_t::LOCAL, c.dst().port());
