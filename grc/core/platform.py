...
class Platform(Element):
        ...
        with Cache(Constants.CACHE_FILE, version = self.config.version) as cache:
            for file_path in self._iter_files_in_block_path(path):
                ...
                if file_path.endswith('.block.yml'):
                    ...
                    scheme = None
                else: # newsched blocks
                    loader = self.load_newsched_block_description
                    scheme = schema_checker.NEWSCHED_BLOCK_SCHEME
    ...
    ##############################################
    # region loaders
    def load_newsched_block_description(self, data, file_path):
        ...
        block_id = data['id'] = data['block'] = data['block'].rstrip('_')
        data['category'] = 'newsched'
