...
int main(int argc, char* argv[])
{
    ...
    desc.add_options()("help,h", "display help")(
        ...
        "nblocks",
        po::value<unsigned int>(&nblocks)->default_value(1),
        "Number of copy blocks")("nthreads",
        ...
        "Buffer Size in bytes")("rt_prio", "Enable Real-time priority")(
        "cpus",
...

    {
        ...
        if (buffer_type == 0) {
            ...
            fg->connect(head, 0, copy_blks[0], 0)->set_custom_buffer(BUFFER_VMCIRC_ARGS);
            for (unsigned int i = 0; i < nblocks - 1; i++) {
                fg->connect(copy_blks[i], 0, copy_blks[i + 1], 0)
        ...
        std::cout << "Initializing MT scheduler with buffer size of " << buffer_size
                  << std::endl;
