...
void scheduler_simplestream::start(scheduler_sync* sync)
{
    sched_sync = sync;
    for (auto& b : d_blocks) {
        b->start();
...
void scheduler_simplestream::stop()
{
...
void scheduler_simplestream::wait()
{
    d_thread.join();
    for (auto& b : d_blocks) {
        b->done();
...
void scheduler_simplestream::run(scheduler_sync* sync)
{
    start(sync);
    wait();
}
...
void scheduler_simplestream::thread_body(scheduler_simplestream* top)
{
    int num_empty = 0;
