...
class scheduler_cuda : public scheduler
{
private:
    std::string _name;
...
public:
    ...
    typedef std::shared_ptr<scheduler_cuda> sptr;
    // static sptr make(const std::string name = "simplestream")
    // {
    //     return std::make_shared<scheduler_cuda>(scheduler_cuda(name));
    // }

    scheduler_cuda(const std::string name = "cuda") : scheduler(name) {}
    ~scheduler_cuda(){
    ...
    void initialize(flat_graph_sptr fg)
    {
        ...
        for (auto e : fg->edges()) {
            ...
            // Case where domain adapter sits at the input to the graph controlled by
            // this scheduler
            if (src_da_cast != nullptr) {
                if (src_da_cast->buffer_location() == buffer_location_t::LOCAL) {
                    // If this DA is src and local, then the crossing is H2D
                    auto buf = cuda_buffer::make(
                        s_fixed_buf_size, e.itemsize(), cuda_buffer_type::H2D);
            ...
            // Case where domain adapter sits at the output to the graph controlled by
            // this scheduler
            else if (dst_da_cast != nullptr) {
                if (dst_da_cast->buffer_location() == buffer_location_t::LOCAL) {
                    // If this DA is dst and local, then the crossing is D2H
                    auto buf = cuda_buffer::make(
                        s_fixed_buf_size, e.itemsize(), cuda_buffer_type::D2H);
                ...
                d_edge_buffers[e.identifier()] =
                    cuda_buffer::make(s_fixed_buf_size, e.itemsize());
            }
...
private:
    ...
    // std::map<block_sptr, std::map<block::io, std::map<int, cuda_buffer::sptr>>>
    //     d_block_buffers; // store the block buffers
    std::map<port_sptr, std::vector<buffer_sptr>> d_block_buffers;
    // std::map<port_sptr, cuda_buffer::sptr> d_block_buffers; // store the block buffers
    std::thread d_thread;
    bool d_thread_stopped = false;

    static void thread_body(scheduler_cuda* top)
    {
        int num_empty = 0;
        ...
        while (!top->d_thread_stopped) {
            ...
            for (auto const& b : top->d_blocks) {
                ...
                for (auto p : b->output_stream_ports()) {
                    ...
                    for (auto p_buf : top->d_block_buffers[p]) {
                        ...
                        // std::cout << top->name() << ":" << b->name() << ":write_info:"
                        // << ready << "-" << write_info.n_items << std::endl;
                        if (!ready)
                            break;
                        bufs.push_back(p_buf);
            ...
            // std::cout << top->name() << ":" << ":state:" << (int) top->state() <<
            // std::endl;;
        }
