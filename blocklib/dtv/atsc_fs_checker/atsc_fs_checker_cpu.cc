#include "atsc_fs_checker_cpu.hh"
...
atsc_fs_checker::sptr atsc_fs_checker::make_cpu(const block_args& args)
{
    return std::make_shared<atsc_fs_checker_cpu>(args);
}

atsc_fs_checker_cpu::atsc_fs_checker_cpu(const block_args& args) : atsc_fs_checker(args)
{
    reset();
}

void atsc_fs_checker_cpu::reset()
{
    d_index = 0;
...
work_return_code_t atsc_fs_checker_cpu::work(std::vector<block_work_input>& work_input,
