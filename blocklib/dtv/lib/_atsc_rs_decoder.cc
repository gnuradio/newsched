...
{
    return std::make_shared<atsc_rs_decoder>();
}

atsc_rs_decoder::atsc_rs_decoder() : gr::sync_block("dtv_atsc_rs_decoder")

{
    add_port(port<uint8_t>::make(
        "in", port_direction_t::INPUT, { ATSC_MPEG_RS_ENCODED_LENGTH }));
    add_port(
...
int atsc_rs_decoder::decode(uint8_t* out, const uint8_t* in)
{
    unsigned char tmp[N];
    int ncorrections;
...
int atsc_rs_decoder::num_errors_corrected() const { return d_nerrors_corrected_count; }

int atsc_rs_decoder::num_bad_packets() const { return d_bad_packet_count; }

int atsc_rs_decoder::num_packets() const { return d_total_packets; }

work_return_code_t atsc_rs_decoder::work(std::vector<block_work_input>& work_input,
