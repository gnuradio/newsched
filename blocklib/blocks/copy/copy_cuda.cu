...
// The block cuda file is just a wrapper for the kernels that will be launched in the work function

__global__ void
apply_copy_kernel(const uint8_t* in, uint8_t* out, int batch_size)
{
...
void apply_copy(const uint8_t* in,
                uint8_t* out,
                int grid_size,
                int block_size,
                cudaStream_t stream)
