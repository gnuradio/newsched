...
#include "complex_to_mag_cuda.hh"
...
complex_to_mag::sptr complex_to_mag::make_cuda(const block_args& args)
{
    return std::make_shared<complex_to_mag_cuda>(args);
}

complex_to_mag_cuda::complex_to_mag_cuda(
    const typename complex_to_mag::block_args& args)
    : complex_to_mag(args), d_vlen(args.vlen)
{
    p_kernel = std::make_shared<cusp::complex_to_mag>();
    p_kernel->cusp::complex_to_mag::occupancy(
...
work_return_code_t
complex_to_mag_cuda::work(std::vector<block_work_input>& work_input,
                          std::vector<block_work_output>& work_output)
{
    ...
    p_kernel->cusp::complex_to_mag::launch(
