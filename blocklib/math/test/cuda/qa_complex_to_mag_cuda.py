...
class test_complex_to_mag_cuda (gr_unittest.TestCase):
    ...
    def test_complex_to_mag_cuda(self):
        tb = self.tb
        src_data = [-2 - 2j, -1 - 1j, -2 + 2j, -1 + 1j,
                    2 - 2j, 1 - 1j, 2 + 2j, 1 + 1j,
                    0 + 0j]

        expected_data = [abs(i) for i in src_data]

        src = blocks.vector_source_c(src_data)
        op = math.complex_to_mag(impl=math.complex_to_mag.cuda)
...
if __name__ == '__main__':
    gr_unittest.run(test_complex_to_mag_cuda)
